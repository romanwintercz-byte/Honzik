...
const STORAGE_KEY = 'loan_ai_tracker_data';
...
const App: React.FC = () => {
  ...
  // Persist to local storage whenever loanDetails change
  useEffect(() => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(loanDetails));
  }, [loanDetails]);
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="text-[10px] text-emerald-600 font-bold bg-emerald-50 px-2 py-1 rounded border border-emerald-100 hidden sm:block">
              DATA ULOŽENA LOKÁLNĚ
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
              <h3 className="font-bold text-slate-800 mb-4">Efekt nastavení</h3>
              <div className="space-y-4">
                ...
                {!amortizationSchedule.actualPayments.length && (
                    <div className="p-3 bg-red-50 text-red-600 text-xs rounded-lg border border-red-100">
                        Pozor: Splátka nepokrývá ani úrok. Půjčka nebude nikdy splacena.
                    </div>
                )}
              </div>
            </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {amortizationSchedule.history.length > 0 ? (
                        amortizationSchedule.history.slice(0, 50).map((h, idx) => (
                        ...
                        <tr>
                          <td colSpan={5} className="px-6 py-8 text-center text-red-500 font-medium">
                            Nepodařilo se vygenerovat splátkový kalendář. Zkontrolujte výši splátky.
                          </td>
                        </tr>
                      )}
                    </tbody>
                  </table>
                  {amortizationSchedule.history.length > 50 && (
                    <div className="px-6 py-3 text-center text-slate-400 text-xs italic bg-slate-50">
                      Zobrazeno prvních 50 transakcí z celkových {amortizationSchedule.history.length}.
                    </div>
                  )}
                </div>
              </div>
